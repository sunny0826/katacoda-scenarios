对 chaosblade 命名空间中，对 `redis-master` 的容器的 6379 端口添加 3000 毫秒访问延迟，延迟时间上下浮动 1000 毫秒。
...
打开 `delay_container_network_by_id.yaml`{{open}} ，将container id 和 pod 名称填入 `container-ids` 和 `names` 中。

执行命令，开始实验：

`kubectl apply -f delay_container_network_by_id.yaml`{{execute}}

**查看实验状态**

执行命令： `kubectl get blade delay-container-network-by-id -o json`{{execute}}
...
可以看到延迟在 3s 左右，结果符合预期。

**停止实验**

执行命令：`kubectl delete -f delay_container_network_by_id.yaml`{{execute}}

或者直接删除 blade 资源：`kubectl delete blade delay-container-network-by-id`{{execute}}